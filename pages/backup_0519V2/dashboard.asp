...
' 실제 데이터 조회 시도
Dim approvalCmd
Set approvalCmd = Server.CreateObject("ADODB.Command")
approvalCmd.ActiveConnection = db
approvalCmd.CommandText = "SELECT A.approval_log_id, A.target_id, CU.store_name, CU.amount, CU.usage_date, A.status " & _
                         "FROM " & dbSchema & ".ApprovalLogs A " & _
                         "JOIN " & dbSchema & ".CardUsage CU ON CU.usage_id = A.target_id " & _
                         "WHERE A.target_table_name = 'CardUsage' AND A.approver_id = ? AND A.status = '대기' " & _
                         "ORDER BY CU.usage_date DESC"
approvalCmd.Parameters.Append approvalCmd.CreateParameter("@approver_id", 200, 1, 30, Session("user_id"))
...
<div class="dashboard-container">
    ...
    <div class="dashboard-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(450px, 1fr)); gap: 20px;">
        ...
        <div class="shadcn-card">
            <div class="shadcn-card-header">
                <h3 class="shadcn-card-title">결재 대기 문서</h3>
            </div>
            <div class="shadcn-card-content">
                <% If approvalRS.EOF Then %>
                ...
                <table class="shadcn-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><%= FormatDate(approvalRS("usage_date")) %></td>
                            <td><%= approvalRS("store_name") %></td>
                            <td><%= FormatNumber(approvalRS("amount")) %></td>
                            <td><span class="shadcn-badge shadcn-badge-secondary"><%= approvalRS("status") %></span></td>
                            <td>
                                <a href="approval_detail.asp?id=<%= approvalRS("target_id") %>" class="shadcn-btn shadcn-btn-sm shadcn-btn-outline">상세보기</a>
                            </td>
                        </tr>
                        <% 
                            approvalRS.MoveNext
