...
' 파라미터 받기
Dim requestId, startDate, endDate, purpose, startLocation, destination, distance, title, toll_fee, parking_fee, total_fee,fuelRate
...
toll_fee = Request.Form("toll_fee")
parking_fee = Request.Form("parking_fee")

Dim fuelRateSQL
fuelRateSQL = "SELECT top 1rate FROM " & dbSchema & ".FuelRate WHERE date <= '" & startDate & "' ORDER BY date DESC"
...
total_cost = (distance * fuelRate) + toll_fee + parking_fee
...
updateSQL = "UPDATE " & dbSchema & ".VehicleRequests SET " & _
            ...
            "toll_fee = " & PreventSQLInjection(toll_fee) & ", " & _
            "parking_fee = " & PreventSQLInjection(parking_fee) & ", " & _
            "total_cost = " & PreventSQLInjection(total_cost) & ", " & _
            "approval_status = '대기'"
